...
class Login extends CI_Controller {
	...
	public function proses_masuk($email, $password)
	{
		...
		else
		{
			if($data['aktif'] == 0)
			{
				$this->session->set_flashdata('message','<div class="alert alert-danger" role="alert">Akun Diblokir, Konfirmasi Administrator</div>');
				$this->load->view('user/form_login');
			}
			else
			{
				if($password == $data['password'])
				{
					...
					$data_log = [
						...
						'nama' => $data["nama"],
						'status' => $status,
						'login'	 => date("Y-m-m h:i:s"),
						'logout' => ""
					];
					
					$input_log	= $this->model_log_activity->input_log($data_log);
